...
import { useContinuousSpinAnimation } from './continuousSpin';
...
const DroneTopDisplay = ({ data }: CanvasElementProps<DroneTopConfig, DroneTopData>) => {
  ...
  const fRightRotorAnimation = useContinuousSpinAnimation(data?.fRightRotorRPM ?? 0);
  const fLeftRotorAnimation = useContinuousSpinAnimation(data?.fLeftRotorRPM ?? 0);
  const bRightRotorAnimation = useContinuousSpinAnimation(data?.bRightRotorRPM ?? 0);
  const bLeftRotorAnimation = useContinuousSpinAnimation(data?.bLeftRotorRPM ?? 0);
